...
- **항의 개수는 최대화 (2의 제곱값 기준, 무관조건 해당 항(x) 포함) , 그룹의 개수는 최소화 하여 K-map 작성.**
...
## 문제 유형 3. 여러 가지 항 경우의 수 발생
### 1. Literal(변수) 이 4개 발생한 경우
![[스크린샷 2023-09-20 오전 2.37.03.png]]

### 2. Literal(변수) 이 3개 발생한 경우
...
- 하지만 **Literal 개수 관점에서 더 적은 Literal 을 가진 결과가 더 잘 간소화** 했다고 볼 수 있다.
	- 이 결과를 도출하기 위해 타 minterm 에서 **이전에 이미 묶여졌던 항을 위주로 묶는다면** (예시 2번처럼) **결과 도출 시 공유하고 있는 변수가 많을 확률**이 높아 다시 한번 간소화 할 수 있어보인다.
...
- **x 값 + y 값 + Carry 값** -> `총 세개의 Input이 존재`한다고 볼 수 있음.
	- Output 은 Sum 과 Carry 의 두 개.
- 반 가산기 두 개로 구현이 가능하다.
